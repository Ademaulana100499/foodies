...
// Menangani props dengan tipe FoodDetail
export async function getServerSideProps(context: GetServerSidePropsContext) {
  const apiUrl: string = process.env.NEXT_PUBLIC_API_URL ?? "";

  // Menggunakan type assertion untuk memastikan params.id ada dan bertipe string
  const { id } = context.params as { id: string };

  // Jika id tidak ditemukan, kembalikan halaman 404
  if (!id) {
    return {
      notFound: true,
    };
  ...
  try {
    const res = await axios.get(`${apiUrl}/foods/${id}`, {
      headers: {
    ...
    // Mengirimkan data food ke halaman
    return { props: { food: res.data.data } };
  } catch (error) {
    return {
      notFound: true, // Jika ada error pada pengambilan data, kembalikan halaman 404
    };
  }
}

// Komponen utama yang menerima props food dengan tipe FoodDetail
const RecipesDetail = ({ food }: FoodDetail) => {
  const router = useRouter();

  // Langkah memasak dummy
  const dummySteps = [
        ...
        <div className="bg-gray-100 min-h-screen">
          <div className="max-w-5xl mt-14 mx-auto px-4 py-8">
            ...
            <div className="bg-white rounded-lg shadow-xl overflow-hidden">
              <Image
                src={food.imageUrl || "/default-image.jpg"} // Menggunakan gambar default jika imageUrl tidak ada
                alt={food.name}
                width={800}
                height={384}
