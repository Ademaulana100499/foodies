...
import { FoodDetail, Food } from "@/types/Recipes.interface";
...
describe("RecipesDetail Page", () => {
  ...
  // Additional Test to ensure component's dynamic rendering works with API Data
  test("renders data dynamically when API data changes", async () => {
    const updatedFood = { ...mockFood, name: "Spaghetti Bolognese" };

    // Mock axios to return updated data
    (axios.get as jest.Mock).mockResolvedValueOnce({
      data: { data: updatedFood },
    });

    render(
      <RouterContext.Provider value={createMockRouter({ query: { id: "1" } })}>
        <RecipesDetail food={updatedFood} />
      </RouterContext.Provider>
    );

    // Check if updated data is rendered
