...
describe("Recipes Page", () => {
  ...
  test("renders recipe items correctly", async () => {
    ...
    // Ensure image for each recipe is rendered with the correct src URL
    expect(screen.getByAltText("Salad").getAttribute("src")).toMatch(
      /\/_next\/image\?url=%2Fpath%2Fto%2Fsalad\.jpg/
    );
    expect(screen.getByAltText("Pasta").getAttribute("src")).toMatch(
      /\/_next\/image\?url=%2Fpath%2Fto%2Fpasta\.jpg/
    );
    expect(screen.getByAltText("Pizza").getAttribute("src")).toMatch(
      /\/_next\/image\?url=%2Fpath%2Fto%2Fpizza\.jpg/
    );
